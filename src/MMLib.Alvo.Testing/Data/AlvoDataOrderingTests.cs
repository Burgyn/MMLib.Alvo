...
public abstract class AlvoDataOrderingTests
{
    ...
    /// <c>title</c> is deterministic and total — the fixture the five facts above need and no earlier fact
    /// in this file has, because <c>LedgerAsync</c>'s single fixed entity has no row-count knob and
    /// carries no caller identity of its own. Extends this suite's existing "build a descriptor/schema pair
    /// by hand, seed it, hand it to <see cref="CreateAsync"/>" idiom rather than adding a second seeding
    /// path: the shape mirrors <c>Fixture</c> below, only parameterised by row count.
    /// </summary>
