...
internal static class ApiSetup
{
    ...
    /// </summary>
    /// <remarks>
    /// Called from <c>AddAlvo</c> and from nowhere else, for the same reason
    /// <c>AddAlvoAuth</c>/<c>AddAlvoRules</c> are: registering a service exposes nothing, so the generated
    /// Data API is <b>on by default</b> and <c>AddDataApi</c> only <em>configures</em> it. Nothing here is
    /// reachable until a host calls <c>MapAlvoDataApi</c> or <c>MapAlvo</c>, which is a separate seam by
    /// design (<c>docs/architecture/extensibility.md</c> rule 10).
    ...
    /// <c>AddAlvo</c> twice — which <c>AddAlvo</c>'s own remarks support — does not report every failure
    /// twice.
    /// </remarks>
    /// <remarks>
    /// <para>
    ...
    /// ApiExplorer and a set of options it never asked for; and it would run once per <c>AddAlvo</c>, which a
    /// host may legitimately call twice.
    /// </para>
    /// <para>
    /// <b>It is deliberately not in <c>AddDataApi</c> either.</b> That seam configures a feature that is
    /// already on — a host may register <c>AddAlvo</c> and call <c>MapAlvo</c> without ever calling it — so
    /// putting the registration there would make enrichment silently depend on a call a host has no reason to
    /// make. Here it applies to every host that registers Alvo at all, and costs a host with no document
    /// nothing but one options setup that is never resolved.
    /// </para>
    /// </remarks>
...
/// <see cref="ApiSetup.AddAlvoApi"/> runs once per <c>AddAlvo</c>, and a host may call <c>AddAlvo</c> twice —
/// its own remarks say so, and two libraries each registering the framework is the ordinary way it happens.
/// <c>AddOpenApi(configure)</c> is additive, so passing the transformer that way registered it twice and the
/// document was enriched twice: the overview paragraph appeared verbatim in <c>info.description</c> twice over.
/// <c>TryAddEnumerable</c> deduplicates on the implementation type, which is exactly the shape every other
/// registration in that method already uses, and
