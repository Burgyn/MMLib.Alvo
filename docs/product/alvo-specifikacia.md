...
| Mutation | Stryker.NET | len bezpečnostné jadro — meria kvalitu adversarial suite | **post-merge na `main`** (path-filtered na zmenený security-core) |
...
Princíp: **contract + snapshot + public-API + arch + API linting bežia rýchlo v každom PR; integračné cez Testcontainers v PR (affected-scoped cez `dotnet-affected`); e2e (Playwright/TeaPie, celé) tiež bežia v PR — pomalšie, ale PR je brána pre všetko okrem mutation. Mutation (path-filtered) je jediná vrstva, ktorá beží post-merge na `main`** — dôvod a cena viď X.3.
...
8. **Mutation testy (Stryker.NET)** — cielené **len na bezpečnostné jadro**: rule engine (mutácia operátora v SQL preklade musí zabiť test), expression evaluátor, tenant izolácia. Meria, či adversarial suite naozaj chytá diery, nie len zvyšuje coverage. Na boilerplate providerov sa nepúšťa (drahé, málo hodnoty). **Beží post-merge na `main`, nie v PR** — jediná vrstva s touto výnimkou (viď X.3).
...
**Rýchlosť CI:** PR je brána pre všetko okrem mutation. Rýchle vrstvy (contract, snapshot, public-API, arch) bežia celé; integračné sú v PR **affected-scoped** cez `dotnet-affected` (len zmenení provideri); e2e celé. Mutation (path-filtered na zmenený security-core) beží **post-merge na `main`**. Všetko zelené v PR = podmienka merge, ktorý robí len človek.
...
- **PR je brána pre všetko okrem mutation — žiadny nightly.** V PR bežia **všetky** testy vrátane e2e, aj keď to trvá minúty. Nič sa neodkladá „na noc". Priamy push do `main` je zakázaný (branch protection); merge robí len človek po review.
- **Jediná výnimka: mutation beží post-merge na `main`.** Pôvodne bola aj ona v PR, pod pravidlom „žiadny post-merge beh". Zmenené vedome: ~20 minút na každom PR dotýkajúcom sa jadra je reálna daň a mutation — na rozdiel od ostatných vrstiev — neodpovedá na otázku „je táto zmena správna?", ale „je suite stále dosť adversarial?". Tá otázka znesie odpoveď o minúty neskôr, lebo náprava je *doplniť test*, nie revertnúť merge. **Cena je explicitná:** na mutation score nič neblokuje merge, takže červený mutation run na `main` je notifikácia, na ktorú musí niekto reagovať — nie brána. Pred rizikovým merge do jadra sa dá spustiť na požiadanie (`workflow_dispatch`). Pre každú *inú* vrstvu pravidlo „žiadny post-merge beh" platí ďalej: rozbitý main nájdený neskoro je presne to, čomu sa vyhýbame.
...
- **Stryker.NET v pipeline:** mutation run nad security-core **post-merge na `main`** (`.github/workflows/mutation.yml`, `on: push: branches: [main]` + `workflow_dispatch`), path-filtered len na zmenené jadro/rule engine/tenancy (na nedotknutom kóde nič nezistí a len zdržuje), s prahom mutation score. Beží cez MTP (`test-runner: mtp` v Stryker configu — VSTest runner MTP testy neobjaví). Run, ktorý otestoval **nula mutantov, padá nahlas** — vacuous green je horší než žiadny gate (stryker-net#3094). Keďže post-merge už nič neblokuje, zlyhanie **otvorí (alebo doplní) tracking issue** so štítkom `mutation-gate` — inak by jediným signálom bol default e-mail tomu, kto klikol merge.
...
- **Lokálne kruhy (agent):** `scripts/test-ring0` (unit, priebežne) → `test-ring1` (+ arch/public-API, po slice) → `test-ring2` (+ integračné affected-scoped + API invariant + Vacuum, pred PR). Plný beh (+ e2e) rieši CI v PR a mutation CI po merge, nie lokálny skript. Rýchlosť kruhu určuje frekvenciu.
