...
| D2a | Spec §0.5 sketches a **configurable admin mount** on both doors — `ALVO_ADMIN__PATH=/admin` standalone, `.UseAdmin(a => a.Path("/alvo/admin").Access(…))` embedded. This design ships **no path option and no per-level `Access(…)`** on the dashboard. | Two separate reasons, and neither is "not yet". **The path:** a routable Razor component's `@page` is a compile-time constant, so a configured mount would have to be a nested pipeline branch, and every link, redirect and `NavigationManager` call inside the dashboard would then have to learn about a prefix the router does not know. ASP.NET Core already has the mechanism — `UsePathBase`, which the standalone host exposes as `Alvo:PathBase` — and a second one beside it is a second place for the two to disagree. **The `Access(…)`:** admission to the dashboard is the descriptor's `access` block, the same block that admits an HTTP caller to the same operations; a per-host override would be the divergent authorization path contract 4 forbids. Argued at length in `host.md` §Configuration; recorded here because a deviation the reader can only find in an architecture note is one they will read as an oversight. |
...
| **The first column added to the identity model had no way of reaching an existing database.** `EnsureTablesAsync` creates the identity tables when absent and does nothing when present, which is right exactly once: the users table's `TenantId` would have been missing from every database an earlier build created, and the first read would have failed at runtime. | Nothing is released, so the cost was zero and the *mechanism* was the gap. `AlvoIdentitySchema` reconciles the model against the database — probe, then add what is missing, in the active provider's own DDL through EF's migration generator, so no engine-specific SQL enters this package. It refuses what it cannot do safely (a required column with no default, on a table with rows) rather than guessing. |
